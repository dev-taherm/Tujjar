...
class TestPlanModel(TestCase):
    def test_create_plan(self):
        plan = Plan.objects.create(
            name="Pro", slug="pro", price=Decimal("29.99"),
            max_products=1000, max_orders=10000,
        )
...
class TestSubscriptionModel(TestCase):
    ...
    def test_create_subscription(self):
        sub = Subscription.objects.create(
            organization=self.org, plan=self.plan, status="active"
        )
...
class TestInvoiceModel(TestCase):
    ...
    def test_create_invoice(self):
        inv = Invoice.objects.create(
            organization=self.org, amount=Decimal("29.99"),
...
class TestPaymentMethodModel(TestCase):
    ...
    def test_create_payment_method(self):
        pm = PaymentMethod.objects.create(
            organization=self.org, method_type="card",
