...
class TestProductModel(TestCase):
    def setUp(self):
        ...
        self.store = Store.objects.create(
            organization=self.org, name="Prod Store", slug="prod-store",
        )
    ...
    def test_create_product(self):
        product = Product.objects.create(
            ...
            status="active", price=Decimal("29.99"), sku="TEST-001",
        )
    ...
    def test_product_slug_unique(self):
        Product.objects.create(
            organization=self.org, store=self.store,
            title="P1", slug="dup-slug", price=Decimal("10"), sku="P1-001",
        )
        with self.assertRaises(Exception):
            Product.objects.create(
                organization=self.org, store=self.store,
                title="P2", slug="dup-slug", price=Decimal("20"), sku="P2-001",
            )
...
class TestCategoryModel(TestCase):
    def setUp(self):
        ...
        self.store = Store.objects.create(
            organization=self.org, name="Cat Store", slug="cat-store",
        )
    ...
    def test_category_hierarchy(self):
        ...
        child = Category.objects.create(
            organization=self.org, store=self.store,
            name="Child", slug="child", parent=parent,
        )
