...
class TestMarketplaceListingModel(TestCase):
    def setUp(self):
        ...
        self.store = Store.objects.create(
            organization=self.org, name="Dev Store", slug="dev-store",
        )
        self.theme = Theme.objects.create(
            organization=self.org, name="Market Theme",
            slug="market-theme", config={},
        )
    
    def test_create_listing(self):
        listing = MarketplaceListing.objects.create(
            developer=self.developer, theme=self.theme,
            slug="market-theme", name="Market Theme",
            description="A great theme", pricing_type="paid",
            price=Decimal("49.99"), status="approved",
        )
...
class TestMarketplaceReviewModel(TestCase):
    def setUp(self):
        ...
        self.store = Store.objects.create(
            organization=self.org, name="Dev Store 2", slug="dev-store-2",
        )
        self.theme = Theme.objects.create(
            organization=self.org, name="Review Theme",
            slug="review-theme", config={},
        )
        self.listing = MarketplaceListing.objects.create(
            developer=self.developer, theme=self.theme,
            slug="review-theme", name="Review Theme",
            description="Theme", status="approved",
        )
    
    def test_create_review(self):
        review = MarketplaceReview.objects.create(
            listing=self.listing, user=self.reviewer,
