...
export default function StorefrontLayout({
  ...
  const logoText = navigation.logo_text || store?.name || "Store";
        ...
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden gap-6 md:flex">
            {navLinks.map((link) => (
              <Link
                key={link.url}
                href={link.url.startsWith("http") ? link.url : `/shop/${slug}${link.url}`}
          ...
          <div className="flex items-center gap-4">
            {navigation.cta_button?.enabled && (
              <Link
                href={navigation.cta_button.url.startsWith("http") ? navigation.cta_button.url : `/shop/${slug}${navigation.cta_button.url}`}
      ...
      <footer className="border-t border-gray-200 bg-gray-50">
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 border-t border-gray-200 pt-4 text-center text-xs text-gray-400">
            {footerConfig.copyright || "Powered by Tujjar"}
          </div>
        </div>
      </footer>
    </div>
