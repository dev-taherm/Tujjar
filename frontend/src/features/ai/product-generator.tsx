        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {t("productContentGenerator")}
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <Input label={t("productTitle")} value={title} onChange={(e) => setTitle(e.target.value)} placeholder={t("productTitlePlaceholder")} />
            <div className="grid grid-cols-2 gap-4">
              <Select label={t("productType")} options={[{ value: "physical", label: t("physical") }, { value: "digital", label: t("digital") }, { value: "service", label: "Service" }]} value={productType} onChange={(e) => setProductType(e.target.value)} />
              <Input label="Price" type="number" step="0.01" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="29.99" />
            </div>
            <Input label={t("category")} value={category} onChange={(e) => setCategory(e.target.value)} placeholder={t("categoryPlaceholder")} />
            <Select label={t("tone")} options={[{ value: "professional", label: t("professional") }, { value: "casual", label: t("casual") }, { value: "luxury", label: t("luxury") }, { value: "playful", label: t("playful") }]} value={tone} onChange={(e) => setTone(e.target.value)} />
            <Button onClick={handleGenerate} isLoading={generate.isPending} className="w-full">
              <Sparkles className="me-2 h-4 w-4" /> {t("generateContent")}
            </Button>
          </CardContent>
        </Card>
        ...
        <div className="space-y-6">
          {result && (
            <>
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between">
                  <CardTitle>{t("seoContent")}</CardTitle>
                ...
                <CardContent className="space-y-3">
                  <div>
                    <p className="text-xs font-medium text-gray-500 mb-1">{t("seoTitle")} ({result.seo_title.length}/60)</p>
                    <p className="text-sm font-medium text-gray-900">{result.seo_title}</p>
                  </div>
                  <div>
                    <p className="text-xs font-medium text-gray-500 mb-1">{t("metaDescription")} ({result.seo_description.length}/160)</p>
          ...
          {!result && !generate.isPending && (
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-sm text-gray-500">{t("enterProductDetails")}</p>
              </CardContent>
            </Card>
          )}
        </div>
